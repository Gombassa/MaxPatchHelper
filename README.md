...
### Note: unused modules kept for a future patch-analyzer feature

Two files in the codebase do nothing in the app today but are kept intentionally:

- **`assistant/validate.py`**: a standalone patch-checker. Given a `.maxpat`-style JSON dict, it verifies things like duplicate box IDs, connections referencing real boxes/inlets/outlets, signal-rate vs. control-rate conflicts on the same `dac~` inlet, and M4L-specific device rules (e.g. an Audio Effect needs `plugin~` + `plugout~`). It never calls Ollama — pure validation logic. It used to run inside the deleted patch generator's retry loop; today it's reachable only via `/api/validate` and the test suite.
- **`assistant/object_reference.py`**: a static reference table for ~180 Max objects — the correct `maxclass` convention for each (e.g. `newobj` with text `cycle~` vs. using the object's own name as maxclass for UI objects like `toggle`) and its exact inlet/outlet count. This used to be embedded inside the deleted patch generator's system prompt; it was pulled out into its own file instead of being deleted, since it's accurate data independent of the generation feature.

Both are the starting point for a planned future "patch analyzer" feature — taking an existing `.maxpat` file (not one generated by an LLM) and checking/describing it using this same logic and data. Neither is wired into any current UI flow yet.
